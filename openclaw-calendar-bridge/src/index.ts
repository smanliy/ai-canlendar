...
type BridgeAction = 'create_run' | 'submit_decision' | 'submit_annotation' | 'list_messages' | 'save_message' | 'clear_messages' | 'query_calendar_events';
...
function resolveBridgeUrl(config: BridgeConfig): string {
...
function readEnvelope(value: unknown): BridgeEnvelope {
  if (!isPlainObject(value)) return {};
  return value as BridgeEnvelope;
}

function extractDataText(value: unknown): string {
...
function resolvePluginConfig(api: { pluginConfig?: Record<string, unknown> }): BridgeConfig {
...
function isWeixinMessage(ctx: { channel?: string; messageProvider?: string; channelId?: string }): boolean {
...
async function bridgeIncomingMessage(params: {
  ...
  const response = await postBridgeRequest(
    resolveBridgeUrl(params.config),
    {
      action: 'create_run',
    ...
    params.config.bridgeToken,
    params.config.timeoutMs ?? 15_000,
    params.signal
  );
    ...
    api.registerTool({
      ...
      async execute(toolCallId, params, signal) {
        ...
        const result = await bridgeToolCall(
          readString(toolParams.action) as BridgeAction,
          toolParams,
          config,
          signal
        );
