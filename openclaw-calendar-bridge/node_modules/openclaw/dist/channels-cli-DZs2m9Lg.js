...
import { i as getRuntimeConfig, u as readConfigFileSnapshot } from "./io-ol5Bj12B.js";
...
import { a as normalizeChannelId, i as listChannelPlugins } from "./registry-BxD52xuY2.js";
...
import { t as commitConfigWithPendingPluginInstalls } from "./plugins-install-record-commit-Fc4xp_dZ.js";
import { t as resolveInstallableChannelPlugin } from "./channel-plugin-resolution-DtVnCatH.js";
...
const channelsCommandsLoader = createLazyImportLoader(() => import("./channels-TzMEgt1Q.js"));
const bundledPackageChannelMetadataLoader = createLazyImportLoader(() => import("./bundled-package-channel-metadata-DfhMP4nI.js"));
...
async function registerChannelsCli(program, argv = process.argv, options = {}) {
	...
	channels.command("list").description("List chat channels (configured by default; pass --all for installable catalog)").option("--all", "Include bundled and installable catalog channels", false).option("--json", "Output JSON", false).action(async (opts) => {
		await runChannelsCommand(async () => {
			const { channelsListCommand } = await import("./list-BQm7QT7f.js");
	...
	channels.command("status").description("Show gateway channel status (use status --deep for local)").option("--channel <name>", `Only show one channel (${formatCliChannelOptions(["all"])})`).option("--probe", "Probe channel credentials", false).option("--timeout <ms>", "Timeout in ms", "10000").option("--json", "Output JSON", false).action(async (opts) => {
		await runChannelsCommand(async () => {
			const { channelsStatusCommand } = await import("./status-CIGit0XC.js");
