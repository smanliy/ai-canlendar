...
function resolveManager(request) {
	const accountId = request.session.source.accountId?.trim();
	if (accountId) return managersByAccountId.get(accountId);
...
async function waitForManager(request) {
	...
	const accountId = request.session.source.accountId?.trim() || void 0;
...
const discordVoiceTranscriptsSourceProvider = {
	...
	async stop(request) {
		...
		const manager = accountId ? managersByAccountId.get(accountId) : [...managersByAccountId.values()][0];
		if (!manager) return {
			ok: false,
			error: "Discord voice manager is not available."
		};
	...
	async status(source) {
		...
		return (accountId ? managersByAccountId.get(accountId) : [...managersByAccountId.values()][0])?.status().map((entry) => ({
			active: entry.ok,
