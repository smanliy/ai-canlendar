...
import { _ as shortenHomePath, m as resolveUserPath } from "./utils-B8fk9j7G.js";
...
const loadOverviewModule = async () => await import("./overview-BRtVqkPt.js");
const loadModelsSharedModule = async () => await import("./shared-BHHA5jbs.js");
const loadConfigCliModule = async () => await import("./config-cli-BnSf5qhE.js");
const loadDoctorModule = async () => await import("./doctor-xbiIRqng.js");
...
async function runGatewayLifecycle(operation) {
	const lifecycle = await import("./lifecycle-CQD4U3FW.js");
	if (operation === "start") {
...
async function executeCrestodianOperation(operation, runtime, opts = {}) {
	...
	if (operation.kind === "plugin-list") {
		...
		await (opts.deps?.runPluginsList ?? (async (pluginRuntime) => {
			const { runPluginsListCommand } = await import("./plugins-list-command-dvexAcrP.js");
	...
	if (operation.kind === "plugin-search") {
		...
		await (opts.deps?.runPluginsSearch ?? (async (query, pluginRuntime) => {
			const { runPluginsSearchCommand } = await import("./plugins-search-command-Cr6QYBXN.js");
	...
	if (operation.kind === "plugin-install") {
		...
		await (opts.deps?.runPluginInstall ?? (async (spec, pluginRuntime) => {
			const { runPluginInstallCommand } = await import("./plugins-install-command-Ag1MYrfe.js");
	...
	if (operation.kind === "plugin-uninstall") {
		...
		await (opts.deps?.runPluginUninstall ?? (async (pluginId, pluginRuntime) => {
			const { runPluginUninstallCommand } = await import("./plugins-uninstall-command-EVatbkvl.js");
	...
	if (operation.kind === "create-agent") {
		...
		const before = await readConfigFileSnapshot();
		const workspace = resolveUserPath(operation.workspace ?? process.cwd());
		await (opts.deps?.runAgentsAdd ?? (await import("./agents.commands.add-Df1hN6MB.js")).agentsAddCommand)({
	...
	if (operation.kind === "status") {
		...
		const { statusCommand } = await import("./status.command-Cf0AgV_Q.js");
	...
	if (operation.kind === "open-tui") {
		...
		const result = await (opts.deps?.runTui ?? (await import("./tui-BGbGZgTK.js")).runTui)({
			local: true,
			session,
			deliver: false,
			historyLimit: 200
