...
type ReplyPayload = {
  ...
  replyToId?: string; /** Internal reply-policy provenance; implicit ids honor single-use reply modes. */
...
type GetReplyOptions = {
  ...
  onAgentRunStart?: (runId: string) => void; /** Called once restart-recovery delivery context has been persisted for the adopted turn. */
  ...
  queuedFollowupLifecycle?: QueuedReplyLifecycle; /** Applies source-channel delivery metadata when a queued follow-up later drains. */
  queuedDeliveryPayloadTransform?: (payload: ReplyPayload) => ReplyPayload; /** Preserves source-channel reply fan-out policy for queued delivery. */
  queuedDeliveryReplyToMode?: ReplyToMode; /** Commits source-channel delivery state after a queued payload is visibly routed. */
  queuedDeliveryPayloadDidDeliver?: (payload: ReplyPayload) => void; /** Re-establishes source-owned async context while a queued follow-up drains. */
  queuedExecutionContext?: <T>(run: () => Promise<T>) => Promise<T>; /** Allow channel-owned progress UI while final/source reply delivery remains message-tool-only. */
...
//#endregion
export { isReplyPayloadTtsSupplement as _, SourceReplyDeliveryMode as a, InputProvenance as c, ReplyPayload as d, ReplyPayloadTtsSupplement as f, isReplyPayloadNonTerminalToolErrorWarning as g, isFastModeAutoProgressPayload as h, ReplyThreadingPolicy as i, FAST_MODE_AUTO_PROGRESS_KIND as l, getReplyPayloadTtsSupplement as m, GetReplyOptions as n, UserTurnTranscriptRecorder as o, buildTtsSupplementMediaPayload as p, PartialReplyPayload as r, PromptImageOrderEntry as s, BlockReplyContext as t, ReplyDeliveryContext as u, markReplyPayloadAsTtsSupplement as v };